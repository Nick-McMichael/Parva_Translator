...
public class Scanner {
	...
	const int halt_Sym = 25;
	const int return_Sym = 26;
	const int read_Sym = 27;
	const int readLine_Sym = 28;
	const int write_Sym = 29;
	const int writeLine_Sym = 30;
	const int plus_Sym = 31;
	const int minus_Sym = 32;
	const int new_Sym = 33;
	const int bang_Sym = 34;
	const int barbar_Sym = 35;
	const int star_Sym = 36;
	const int slash_Sym = 37;
	const int percent_Sym = 38;
	const int andand_Sym = 39;
	const int equalequal_Sym = 40;
	const int bangequal_Sym = 41;
	const int less_Sym = 42;
	const int lessequal_Sym = 43;
	const int greater_Sym = 44;
	const int greaterequal_Sym = 45;
	const int equal_Sym = 46;
	const int NOT_SYM = 47;
	// pragmas
	const int CodeOn_Sym = 48;
	const int CodeOff_Sym = 49;
	const int DebugOn_Sym = 50;
	const int DebugOff_Sym = 51;
	const int WarningsOn_Sym = 52;
	const int WarningsOff_Sym = 53;
	const int StackDump_Sym = 54;
	const int HeapDump_Sym = 55;
	const int TableDump_Sym = 56;
	...
	static Token NextToken() {
		...
		switch (state) {
			...
			case 21:
				if (ch == 'C') { buf.Append(ch); NextCh(); goto case 22; }
				else if (ch == 'D') { buf.Append(ch); NextCh(); goto case 23; }
				else if (ch == 'W') { buf.Append(ch); NextCh(); goto case 24; }
				else if (ch == 'S') { buf.Append(ch); NextCh(); goto case 25; }
				else if (ch == 'H') { buf.Append(ch); NextCh(); goto case 18; }
				else { t.kind = noSym; goto done; }
			case 22:
			...
			case 24:
				...
				else if (ch == '-') { buf.Append(ch); NextCh(); goto case 16; }
				else { t.kind = noSym; goto done; }
			case 25:
				if (ch == 'D') { buf.Append(ch); NextCh(); goto case 17; }
				else if (ch == 'T') { buf.Append(ch); NextCh(); goto case 20; }
				else { t.kind = noSym; goto done; }
			case 26:
