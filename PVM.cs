...
// K. Bradshaw, 2019
// As supplied for the last practical with addition of Pascal-style for-loop
// 28/10/2019
...
namespace Parva {
  ...
  // The interpreters and utility methods
      ...
      do {
        ...
        switch (cpu.ir) {         // execute
          ...
          case PVM.lda_5:         // push local address 5
            ...
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.lda_6:         // push local address 6
            adr = cpu.fp - 7;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.lda_7:         // push local address 7
            adr = cpu.fp - 8;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.lda_8:         // push local address 8
            adr = cpu.fp - 9;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.lda_9:         // push local address 9
            adr = cpu.fp - 10;
            if (InBounds(adr)) Push(adr);
            break;
          ...
          case PVM.ldg_5:         // push local address 5
            ...
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.ldg_6:         // push local address 6
            adr = cpu.gp - 7;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.ldg_7:         // push local address 7
            adr = cpu.gp - 8;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.ldg_8:         // push local address 8
            adr = cpu.gp - 9;
            if (InBounds(adr)) Push(adr);
            break;
          case PVM.ldg_9:         // push local address 9
            adr = cpu.gp - 10;
            if (InBounds(adr)) Push(adr);
            break;
          ...
          case PVM.ldl_5:         // push value of local variable 5
            ...
            if (InBounds(adr)) Push(mem[adr]);
            break;
          case PVM.ldl_6:         // push value of local variable 6
            adr = cpu.fp - 7;
            if (InBounds(adr)) Push(mem[adr]);
            break;
          case PVM.ldl_7:         // push value of local variable 7
            adr = cpu.fp - 8;
            if (InBounds(adr)) Push(mem[adr]);
            break;
          case PVM.ldl_8:         // push value of local variable 8
            adr = cpu.fp - 9;
            if (InBounds(adr)) Push(mem[adr]);
            break;
          case PVM.ldl_9:         // push value of local variable 9
            adr = cpu.fp - 10;
            if (InBounds(adr)) Push(mem[adr]);
            break;
          ...
          case PVM.stl_5:         // pop to local variable 5
            ...
            if (InBounds(adr)) mem[adr] = Pop();
            break;
          case PVM.stl_6:         // pop to local variable 6
            adr = cpu.fp - 7;
            if (InBounds(adr)) mem[adr] = Pop();
            break;
          case PVM.stl_7:         // pop to local variable 7
            adr = cpu.fp - 8;
            if (InBounds(adr)) mem[adr] = Pop();
            break;
          case PVM.stl_8:         // pop to local variable 8
            adr = cpu.fp - 9;
            if (InBounds(adr)) mem[adr] = Pop();
            break;
          case PVM.stl_9:         // pop to local variable 9
            adr = cpu.fp - 10;
            if (InBounds(adr)) mem[adr] = Pop();
            break;
    ...
    // Initializes stack machine
      ...
      mnemonics[PVM.efd]      = "EFD";
      mnemonics[PVM.efu]      = "EFU";
      ...
      mnemonics[PVM.lda_6]    = "LDA_6";
      mnemonics[PVM.lda_7]    = "LDA_7";
      mnemonics[PVM.lda_8]    = "LDA_8";
      mnemonics[PVM.lda_9]    = "LDA_9";
      ...
      mnemonics[PVM.ldc_6]    = "LDC_6";
      mnemonics[PVM.ldc_7]    = "LDC_7";
      mnemonics[PVM.ldc_8]    = "LDC_8";
      mnemonics[PVM.ldc_9]    = "LDC_9";
      ...
      mnemonics[PVM.ldg_6]    = "LDG_6";
      mnemonics[PVM.ldg_7]    = "LDG_7";
      mnemonics[PVM.ldg_8]    = "LDG_8";
      mnemonics[PVM.ldg_9]    = "LDG_9";
      ...
      mnemonics[PVM.ldl_6]    = "LDL_6";
      mnemonics[PVM.ldl_7]    = "LDL_7";
      mnemonics[PVM.ldl_8]    = "LDL_8";
      mnemonics[PVM.ldl_9]    = "LDL_9";
      ...
      mnemonics[PVM.sfd]      = "SFD";
      mnemonics[PVM.sfu]      = "SFU";
      ...
      mnemonics[PVM.stl_6]    = "STL_6";
      mnemonics[PVM.stl_7]    = "STL_7";
      mnemonics[PVM.stl_8]    = "STL_8";
      mnemonics[PVM.stl_9]    = "STL_9";
